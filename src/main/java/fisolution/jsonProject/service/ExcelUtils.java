...
public class ExcelUtils {
     ...
     */
    public <T> List<T> excelToJson(File file, Class<T> clazz) throws NoSuchMethodException {
        ...
        excelFile.forEach(lines -> {
            for(int i = 1; i < lines.size(); i++){
                ...
                try {
                    T dataLine = (T) constructor.newInstance();
        ...
        try{
            ...
            while(rowNum <= dataList.size()){
                ...
                Arrays.stream(data.getDeclaredFields()).forEach(field -> {
                    ...
                    try {
                        ExcelColumn declaredAnnotation = field.getDeclaredAnnotation(ExcelColumn.class);
                        opt = Optional.ofNullable(declaredAnnotation.name());
                    } catch (Exception e) {
                    ...
                    try {
                        ...
                        if(fieldType.equals(String.class)){
                            ...
                            cell.setCellValue((Integer) object.get());
    ...
    private List<List<List>> analyzeExcel(File file) {
